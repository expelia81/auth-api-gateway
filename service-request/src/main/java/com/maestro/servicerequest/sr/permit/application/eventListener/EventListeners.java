...
public class EventListeners {
    ...
    private final SubmitDataAccessPort submitDataAccessPort;
    ...
    public void submitRequest(Resource inputResource) {
              Flux.interval(Duration.ofSeconds(2))
                .flatMap(i -> {
                  ...
                  .filter(resource -> resource.getResourceStatus().equals(ResourceStatusCode.REQUESTED.code) || resource.getResourceStatus().equals(ResourceStatusCode.FAILED.code))
                .flatMap(resource -> { // 피날레 작업 완료 확인시 DB에 상태 업데이트
                    log.info("피날레 작업종료. \n작업결과 : " + (resource.getResourceStatus()==ResourceStatusCode.COMPLETED.code?"성공":"실패"));
                    return submitDataAccessPort.updateStatus(resource);
                }).take(1).subscribe();
    }
}
