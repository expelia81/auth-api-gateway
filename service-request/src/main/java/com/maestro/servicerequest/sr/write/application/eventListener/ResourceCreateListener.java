package com.maestro.servicerequest.sr.write.application.eventListener;
...
public class ResourceCreateListener {
    ...
    public void resourceCreate(Resource inputResource) {
        ...
        Flux.range(1, 1000)
            ...
            /* 요청이 완료 / 혹은 실패일 때,  */
            .filter(resource -> resource.getResourceStatus().equals(ResourceStatusCode.COMPLETED.getCode()) || resource.getResourceStatus().equals(ResourceStatusCode.FAILED.getCode()))
            .flatMap(resource -> { // 피날레 작업 완료 확인시 DB에 상태 업데이트
                log.info("피날레 작업종료. \n작업결과 : " + (resource.getResourceStatus()==ResourceStatusCode.COMPLETED.getCode()?"성공":"실패"));
                ...
                if (e instanceof ServerNotRespondedException){
                    ...
                    return permitDataAccessPort.updateStatus(inputResource.updateStatus(ResourceStatusCode.NOT_RESPONDED.getCode()));
                } else {
